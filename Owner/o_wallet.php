...
if (isset($_POST['save_btn'])) {
	if ($_POST['action'] == 'wd') {
		if ($_POST['amount_in'] <= $_SESSION['wallet']) {
			mysqli_query($con, "UPDATE owner SET wallet=wallet-".$_POST['amount_in']." WHERE user_id='".$_SESSION['user_id']."'");
			$_SESSION['wallet'] = 0;
			header("Location: o_wallet.php?wd=done");
		}
		else {
			header("Location: o_wallet.php?wd=no");
		}
	}
	if ($_POST['action'] == 'dp') {
		mysqli_query($con, "UPDATE owner SET wallet=wallet+".$_POST['amount_in']." WHERE user_id='".$_SESSION['user_id']."'");
		$owner = mysqli_fetch_array(mysqli_query($con, "SELECT wallet FROM owner WHERE user_id='".$_SESSION['user_id']."'"));
		$_SESSION['wallet'] = $owner['wallet'];
		header("Location: o_wallet.php?dp=done");
	}
}
